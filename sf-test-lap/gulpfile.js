...
var pxml = require('pxml').PackageXML;
dotenv.config();
...
const controllerName = process.env.CONTROLLER;
...
<Package xmlns="http://soap.sforce.com/2006/04/metadata">
    ...
    <types>
        <members>${controllerName}</members>
        <name>ApexClass</name>
    </types>
...
gulp.task('page_to_prod', function () {
  gulp.src([distPath+'/index.html'])
    ...
    .pipe(replace('<html lang="en">', `<apex:page ${controller} ${otherPageAttrs} ${extensions}>`))
    ...
    .pipe(replace(`<script src="`, `<script type="text/javascript" src="{!URLFOR($Resource.${resources}, '`))
    .pipe(replace(`.js" defer></script>`, `.js')}" async="defer"></script>`))
...
gulp.task('build-apex', () => {
  return gulp.src('./apex-classes/*.*')
    .pipe(gulp.dest('package/classes/'));
});

gulp.task('build-static', ['create-package', 'staticresources', 'build-apex'])
gulp.task('build-package', ['create-package', 'page_to_prod', 'staticresources', 'build-apex'])
gulp.task('build-dev-package', ['create-package', 'page_to_dev', 'build-apex'])
